        ...
        for Location in possibleRankLocations:
            if Location in JSON:
                if Location == 'rank' and JSON[Location] == 'NORMAL':
                    ...
                    playerRank['rank'] = dirtyRank.replace("SUPERSTAR", "MVP++").replace("YOUTUBER", "Hypixel YouTuber").replace("ADMIN", "Hypixel Admin").replace("HELPER", "Hypixel Helper").replace("MODERATOR", "Hypixel Mod")
